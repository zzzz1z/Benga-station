...
const PlaylistDetails: React.FC<{ id?: string }> = (props) => {
  ...
  const fetchPlaylist = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await authedFetch(`${process.env.NEXT_PUBLIC_API_URL}/api/playlist/${id}`);
      if (!res.ok) { setError('Playlist não encontrada.'); return; }
      const data = await res.json();
      setPlaylist(data);
      setSongs((data.playlist_songs ?? []).map((item: any) => item.Songs).filter(Boolean));
    } catch {
              ...
              >
                ...
                <div className="relative w-full h-full bg-red-500 p-[2px]"
                  ...
                  <div className="relative w-full h-full bg-neutral-900 overflow-hidden" style={{ clipPath: GAMER_CUT }}>
                    ...
                    <div className="absolute inset-0 bg-black/60 opacity-0 flex flex-col items-center justify-center gap-y-2">
                      {uploadingCover ? (
