...
console.log('upsert result:', { song_id, status: upsertRes.status });
  ...
  const handleInfo = async (e: React.MouseEvent) => {
    ...
    const json = await res.json();
    if (json.id) router.push(`/songs?id=${json.id}`);
...
>
        ...
        <div
          className="hidden md:flex items-center gap-x-3 flex-shrink-0 pr-2 opacity-0"
          onClick={e => e.stopPropagation()}
        >
          <button onClick={handleLike} className="text-neutral-400 flex items-center justify-center w-5 h-5">
            {isLiked ? <AiFillHeart size={16} className="text-red-500" /> : <AiOutlineHeart size={16} />}
          </button>
          <button onClick={handlePlaylistClick} className="text-neutral-400 flex items-center justify-center w-5 h-5">
            <MdPlaylistAdd size={18} />
          </button>
          <button onClick={handleInfo} className="text-neutral-400 flex items-center justify-center w-5 h-5">
          ...
          <div className="flex items-center justify-center w-5 h-5">
            {isLoading
              ...
              : <FaPlay size={12} className={`transition ${isPlaying ? 'text-red-400' : 'text-neutral-400'}`} />
            }
          </div>
        </div>
          ...
          <div className="relative" ref={menuRef}>
            ...
            {showMenu && (
              ...
              <div className="absolute right-0 bottom-full mb-1 w-44 bg-neutral-800 rounded-xl shadow-xl z-50 overflow-hidden border border-neutral-700">
                <button onClick={handleLike} className="flex items-center gap-x-3 w-full px-4 py-3 text-sm text-white">
                ...
                <button onClick={handlePlaylistClick} className="flex items-center gap-x-3 w-full px-4 py-3 text-sm text-white">
                  <MdPlaylistAdd size={16} className="text-neutral-400" />
                  Adicionar à playlist
                </button>
                <button onClick={handleInfo} className="flex items-center gap-x-3 w-full px-4 py-3 text-sm text-white">
